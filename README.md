# 💫 About Me
🔭 Currently building **NarutoBot** — a real-time multiplayer RPG platform with matchmaking, PvP battles, clans, and an in-game economy  

👯 Open to collaborating on **backend systems, real-time applications, and system design projects**

🌱 Currently learning **distributed systems and Go**

💬 Ask me about **backend development, APIs, databases, and scalable architectures**

⚡ Fun fact: I enjoy building production systems and solving algorithmic problems (1800+ LeetCode rating)

---

# 🌐 Socials
[![LinkedIn](https://img.shields.io/badge/LinkedIn-0077B5?logo=linkedin&logoColor=white)](https://linkedin.com/in/tejasfirange)
[![Email](https://img.shields.io/badge/Email-D14836?logo=gmail&logoColor=white)](mailto:tejasfirange02@gmail.com)

---

# 💻 Tech Stack

### Languages
![Python](https://img.shields.io/badge/python-3670A0?style=for-the-badge&logo=python&logoColor=ffdd54)
![JavaScript](https://img.shields.io/badge/javascript-323330?style=for-the-badge&logo=javascript&logoColor=F7DF1E)
![Java](https://img.shields.io/badge/java-ED8B00?style=for-the-badge&logo=openjdk&logoColor=white)
![C++](https://img.shields.io/badge/c++-00599C?style=for-the-badge&logo=c%2B%2B&logoColor=white)

### Backend
![NodeJS](https://img.shields.io/badge/node.js-6DA55F?style=for-the-badge&logo=node.js&logoColor=white)
![FastAPI](https://img.shields.io/badge/FastAPI-005571?style=for-the-badge&logo=fastapi)
![Express.js](https://img.shields.io/badge/express.js-404d59?style=for-the-badge&logo=express)

### Frontend
![React](https://img.shields.io/badge/react-20232a?style=for-the-badge&logo=react&logoColor=61DAFB)
![TailwindCSS](https://img.shields.io/badge/tailwindcss-38B2AC?style=for-the-badge&logo=tailwind-css&logoColor=white)

### Databases
![Postgres](https://img.shields.io/badge/postgres-316192?style=for-the-badge&logo=postgresql&logoColor=white)
![MongoDB](https://img.shields.io/badge/MongoDB-4ea94b?style=for-the-badge&logo=mongodb&logoColor=white)
![Redis](https://img.shields.io/badge/redis-DD0031?style=for-the-badge&logo=redis&logoColor=white)

### DevOps & Tools
![Docker](https://img.shields.io/badge/docker-0db7ed?style=for-the-badge&logo=docker&logoColor=white)
![AWS](https://img.shields.io/badge/AWS-FF9900?style=for-the-badge&logo=amazon-aws&logoColor=white)
![Nginx](https://img.shields.io/badge/nginx-009639?style=for-the-badge&logo=nginx&logoColor=white)
![Git](https://img.shields.io/badge/git-F05033?style=for-the-badge&logo=git&logoColor=white)

**# 🚀 Featured Projects

### 🥷 NarutoBot – Real-Time Multiplayer RPG Platform
A production-ready multiplayer RPG system with matchmaking, PvP battles, clans, ranking seasons, and an in-game economy.

**Highlights**
- Supports **2000+ active users**
- Real-time battle system with **stateful combat engine**
- **MMR ranking, seasons, clans, inventory, and economy systems**
- Optimized with **asynchronous workers and background jobs**

**Tech Stack:** Node.js, PostgreSQL, Redis, WebSockets

---

### 🏥 MediConnect – AI Healthcare Assistance Platform
An AI-powered healthcare triage platform designed to assist patients in symptom assessment and doctor consultations.

**Highlights**
- AI-based **symptom analysis and health guidance**
- **Doctor consultation queue** with claim-based conflict prevention
- Secure **JWT authentication and password recovery workflows**

**Tech Stack:** React, Node.js, Express, PostgreSQL

---

# 📊 GitHub Stats
![](https://github-readme-stats.vercel.app/api?username=tejasfirange&theme=shadow_blue&hide_border=false&include_all_commits=false&count_private=true)

![](https://nirzak-streak-stats.vercel.app/?user=tejasfirange&theme=shadow_blue&hide_border=false)

![](https://github-readme-stats.vercel.app/api/top-langs/?username=tejasfirange&theme=shadow_blue&hide_border=false&layout=compact)

---

![](https://visitcount.itsvg.in/api?id=tejasfirange&icon=0&color=0)
